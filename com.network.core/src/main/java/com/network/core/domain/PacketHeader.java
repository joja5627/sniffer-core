...
import com.network.core.util.IpV4Utils;
...
try {
    if (!Objects.isNull(packet) && !Objects.isNull(networkDevice)) {
        ...
        if (protocol == PayloadProtocol.ARP) {
            ...
            if (!Objects.isNull(layer2Header)) {
        ...
        if(Objects.nonNull(srcIpAddress) && Objects.nonNull(dstIpAddress)){
    ...
    } else if (!Objects.isNull(packet)) {
        System.out.println("Packet Null");

    } else if (!Objects.isNull(networkDevice)) {
